  ...
  const getFleetAnalysis = useCallback(async (devices, positions) => {
    ...
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are an advanced Fleet Intelligence AI for GeoSurePath. Analyze the provided device data and provide 3-4 concise, actionable insights regarding fleet health, behavior, and maintenance. Focus on AIS140 compliance and zero-lag operational efficiency.',
            },
            {
              role: 'user',
      ...
      const localInsights = [
        ...
        stats.moving > 0
          ? `${stats.moving} devices are currently in motion.`
          : 'No active movement detected.',
        stats.lowBattery > 0
          ? `Warning: ${stats.lowBattery} devices report low battery (<20%).`
          : 'All online batteries within safe range.',
        stats.nonCompliant > 0
          ? `Compliance Alert: ${stats.nonCompliant} devices are not yet AIS140 certified.`
          : 'All monitored devices are AIS140 compliant.',
      ];
