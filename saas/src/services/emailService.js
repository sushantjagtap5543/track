...
const sendWelcomeEmail = async (user) => {
  const html = `
    <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
      ...
      <p style="font-size: 0.8rem; color: #777;">&copy; 2026 GeoSurePath Global Tracking. All rights reserved.</p>
    </div>
  `;
  return sendEmail(user.email, 'Welcome to GeoSurePath!', html);
};

const sendBillingAlert = async (user, amount, dueDate) => {
  const html = `
    <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
      <h2 style="color: #ef4444;">Billing Reminder: GeoSurePath</h2>
      <p>Hello ${user.name},</p>
  ...
  `;
  return sendEmail(user.email, 'Billing Alert: GeoSurePath', html);
};
