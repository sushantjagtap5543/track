...
Currently the benchmark compares the performance against the hiredis parser:
    ...
    HIREDIS:   $ multiple chunks in a bulk string x 994,387 ops/sec Â±0.22% (554 runs sampled)
    JS PARSER: $ multiple chunks in a bulk string x 1,010,728 ops/sec Â±0.28% (559 runs sampled)
    HIREDIS BUF:   $ multiple chunks in a bulk string x 648,742 ops/sec Â±0.80% (526 runs sampled)
    JS PARSER BUF: $ multiple chunks in a bulk string x 1,728,849 ops/sec Â±0.41% (555 runs sampled)

    HIREDIS:   + multiple chunks in a string x 1,861,132 ops/sec Â±0.18% (564 runs sampled)
    JS PARSER: + multiple chunks in a string x 2,131,892 ops/sec Â±0.31% (558 runs sampled)
    HIREDIS BUF:   + multiple chunks in a string x 965,132 ops/sec Â±0.58% (521 runs sampled)
    JS PARSER BUF: + multiple chunks in a string x 2,304,482 ops/sec Â±0.31% (559 runs sampled)

    HIREDIS:   $ 4mb bulk string x 269 ops/sec Â±0.56% (452 runs sampled)
    JS PARSER: $ 4mb bulk string x 763 ops/sec Â±0.25% (466 runs sampled)
    HIREDIS BUF:   $ 4mb bulk string x 336 ops/sec Â±0.59% (459 runs sampled)
    JS PARSER BUF: $ 4mb bulk string x 994 ops/sec Â±0.36% (482 runs sampled)

    HIREDIS:   + simple string x 2,504,305 ops/sec Â±0.19% (563 runs sampled)
    JS PARSER: + simple string x 5,121,952 ops/sec Â±0.30% (560 runs sampled)
    HIREDIS BUF:   + simple string x 1,122,899 ops/sec Â±0.52% (516 runs sampled)
    JS PARSER BUF: + simple string x 5,907,323 ops/sec Â±0.23% (562 runs sampled)

    HIREDIS:   : integer x 2,461,376 ops/sec Â±0.14% (561 runs sampled)
    JS PARSER: : integer x 18,543,688 ops/sec Â±0.19% (539 runs sampled)
    JS PARSER STR: : integer x 14,149,305 ops/sec Â±0.24% (561 runs sampled)

    HIREDIS:   : big integer x 2,114,270 ops/sec Â±0.15% (561 runs sampled)
    JS PARSER: : big integer x 10,794,439 ops/sec Â±0.25% (560 runs sampled)
    JS PARSER STR: : big integer x 4,594,807 ops/sec Â±0.24% (558 runs sampled)

    HIREDIS:   * array x 45,597 ops/sec Â±0.23% (565 runs sampled)
    JS PARSER: * array x 68,396 ops/sec Â±0.30% (563 runs sampled)
    HIREDIS BUF:   * array x 14,726 ops/sec Â±0.39% (498 runs sampled)
    JS PARSER BUF: * array x 80,961 ops/sec Â±0.25% (561 runs sampled)

    HIREDIS:   * big nested array x 212 ops/sec Â±0.17% (511 runs sampled)
    JS PARSER: * big nested array x 243 ops/sec Â±0.21% (496 runs sampled)
    HIREDIS BUF:   * big nested array x 207 ops/sec Â±0.37% (430 runs sampled)
    JS PARSER BUF: * big nested array x 297 ops/sec Â±1.10% (421 runs sampled)

    HIREDIS:   - error x 168,761 ops/sec Â±0.28% (559 runs sampled)
    JS PARSER: - error x 424,257 ops/sec Â±0.28% (557 runs sampled)
