...
const MapPositions = ({
  ...
  useEffect(() => {
    ...
    [id, selected].forEach((source) => {
      ...
      // Ignition-off highlight ring layer
      map.addLayer({
        id: `ignition-off-${source}`,
        type: 'circle',
        source,
        filter: ['all', ['!has', 'point_count'], ['==', ['get', 'ignitionOff'], true]],
        paint: {
      ...
      map.addLayer({
        id: `ignition-off-pulse-${source}`,
        type: 'circle',
        source,
        filter: ['all', ['!has', 'point_count'], ['==', ['get', 'ignitionOff'], true]],
        paint: {
          'circle-radius': 28 * iconScale,
          'circle-color': 'transparent',
    ...
    return () => {
      ...
      [id, selected].forEach((source) => {
        ...
        if (map.getLayer(`ignition-off-${source}`)) {
          map.removeLayer(`ignition-off-${source}`);
        }
