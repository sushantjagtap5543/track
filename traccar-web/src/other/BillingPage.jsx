...
const BillingPage = () => {
    ...
    const fetchMyBill = async () => {
        try {
            ...
            } else { setError("Failed to fetch billing cycle data."); }
    ...
    const handleSettleForUser = async (targetId, planId, total) => {
        ...
        try {
            ...
            if (res.ok) { alert("Sovereign Settle Applied."); fetchAnalyticsAndLedger(); }
    ...
    if (loading) return (
        <Box sx={{ display: 'flex', justifyContent: 'center', p: 10, bgcolor: '#0f172a', height: '100vh', alignItems: 'center', flexDirection: 'column', gap: 2 }}>
            <CircularProgress size={60} thickness={4} sx={{ color: '#3b82f6' }} />
            <Typography variant="h6" sx={{ color: 'white', fontWeight: 900, opacity: 0.6 }}>INITIALIZING SOVEREIGN LEDGER...</Typography>
        </Box>
    );
    ...
    return (
        <Container maxWidth="xl" sx={{ mt: 4, mb: 10 }}>
                ...
                <Paper sx={{ mb: 4, borderRadius: '24px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)' }}>
                    <Tabs 
                        ...
                        sx={{ 
                            ...
                            '& .MuiTab-root': { fontWeight: 900, fontSize: '1rem', py: 3, color: 'rgba(255,255,255,0.5)' },
                ...
                <Box>
                        ...
                        <Grid item xs={12} md={3}>
                            <Card sx={{ borderRadius: '24px', background: 'linear-gradient(135deg, rgba(59,130,246,0.2) 0%, rgba(29,78,216,0.1) 100%)', border: '1px solid #3b82f6' }}>
                                <CardContent sx={{ p: 4 }}>
                                    ...
                                    <Typography variant="h3" fontWeight={900}>₹{analytics?.summary?.totalRevenue || 0}</Typography>
                                </CardContent>
                            </Card>
                        </Grid>
                        <Grid item xs={12} md={3}>
                            <Card sx={{ borderRadius: '24px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)' }}>
                                <CardContent sx={{ p: 4 }}>
                                    ...
                                    <Typography variant="h3" fontWeight={900}>{analytics?.summary?.totalUsers || 0}</Typography>
                                </CardContent>
                            </Card>
                        </Grid>
                        <Grid item xs={12} md={3}>
                            <Card sx={{ borderRadius: '24px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)' }}>
                                <CardContent sx={{ p: 4 }}>
                                    ...
                                    <Typography variant="h3" fontWeight={900}>{analytics?.summary?.totalDevices || 0}</Typography>
                                </CardContent>
                            </Card>
                        </Grid>
                        <Grid item xs={12} md={3}>
                            <Card sx={{ borderRadius: '24px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)' }}>
                                <CardContent sx={{ p: 4 }}>
                                    ...
                                    <Typography variant="h3" fontWeight={900} color="success.main">{analytics?.summary?.churnRate || 2.5}%</Typography>
                                </CardContent>
                            </Card>
                        </Grid>
                    </Grid>
                    ...
                    <Paper sx={{ p: 4, borderRadius: '24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.1)' }}>
                        ...
                        <Grid container spacing={2}>
                            {analytics?.monthlyBreakdown ? Object.entries(analytics.monthlyBreakdown).map(([month, val]) => (
                                ...
                                <Grid item xs={12}><Typography variant="body2" sx={{ opacity: 0.5 }}>Synchronizing Time-Series Data...</Typography></Grid>
                            )}
                        </Grid>
                    </Paper>
                </Box>
                ...
                <Paper sx={{ p: 4, borderRadius: '24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.1)' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
                        ...
                        <TextField 
                            ...
                            sx={{ width: 400, '& .MuiOutlinedInput-root': { borderRadius: '12px', background: 'rgba(255,255,255,0.03)' } }}
                                        ...
                                        <TableCell>
                                            <Chip 
                                                ...
                                                sx={{ 
                                                    fontWeight: 900, 
                                                    width: 90, 
                                                    bgcolor: u.status === 'PAID' ? 'rgba(74,222,128,0.2)' : (u.status === 'GRACE' ? 'rgba(250,204,21,0.2)' : 'rgba(248,113,113,0.2)'),
                                                    color: u.status === 'PAID' ? '#4ade80' : (u.status === 'GRACE' ? '#facc15' : '#f87171')
                                                }}
                                            />
                                        </TableCell>
                                        <TableCell align="right" sx={{ fontWeight: 900 }}>₹{u.totalDue?.toFixed(2)}</TableCell>
            ...
            {(tabIndex === 2 || !admin) && (
                <Paper elevation={3} sx={{ p: 4, borderRadius: '24px', background: 'rgba(255,255,255,0.01)', backdropFilter: 'blur(30px)', border: '1px solid rgba(255,255,255,0.05)' }}>
                            ...
                            <Grid item xs={12} md={4} key={plan.id}>
                                ...
                                >
                                    <CardContent sx={{ p: 4, textAlign: 'center' }}>
                                        <Typography variant="h6" fontWeight={800} sx={{ opacity: 0.7 }}>{plan.name}</Typography>
                                        ...
                                        <Box sx={{ textAlign: 'left', opacity: 0.5 }}>
                                            ...
                                            <Typography variant="caption" display="block">● Provisioning for {deviceCount} Units</Typography>
                                        </Box>
                                    </CardContent>
                                </Card>
                            </Grid>
                    ...
                    <Paper sx={{ p: 4, borderRadius: '24px', background: 'rgba(255,255,255,0.03)', mb: 5, border: '1px solid rgba(255,255,255,0.05)' }}>
                            ...
                            <Grid item xs={6} md={3}>
                                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                    ...
                                    <Box>
                                        ...
                                        <Typography fontWeight={900}>₹{((currentPlan?.breakdown?.basic || 0) * deviceCount).toFixed(2)}</Typography>
                                    </Box>
                                </Box>
                            </Grid>
                            <Grid item xs={6} md={3}>
                                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                    ...
                                    <Box>
                                        ...
                                        <Typography fontWeight={900}>₹{((currentPlan?.breakdown?.server || 0) * deviceCount).toFixed(2)}</Typography>
                                    </Box>
                                </Box>
                            </Grid>
                            <Grid item xs={6} md={3}>
                                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                    ...
                                    <Box>
                                        ...
                                        <Typography fontWeight={900}>₹{((currentPlan?.breakdown?.cloud || 0) * deviceCount).toFixed(2)}</Typography>
                                    </Box>
                                </Box>
                            </Grid>
                            <Grid item xs={6} md={3}>
                                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                                    ...
                                    <Box>
                                        ...
                                        <Typography fontWeight={900}>₹{((currentPlan?.breakdown?.gst || 0) * deviceCount + ((bill?.totalDue || 0) * 0.18)).toFixed(2)}</Typography>
                                    </Box>
                                </Box>
                            </Grid>
                        </Grid>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Box>
                                ...
                                <Typography variant="caption" sx={{ opacity: 0.5 }}>Includes accrued debt of ₹{bill?.totalDue || 0}</Typography>
                            </Box>
                            <Button 
                                variant="contained" 
                                size="large" 
                    ...
                    <TableContainer>
                        <Table>
                            <TableHead>
                                <TableRow sx={{ '& th': { borderBottom: '2px solid rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.5)', fontWeight: 800 } }}>
                            ...
                            <TableBody>
                                {bill?.history?.length > 0 ? bill.history.map((entry, idx) => (
                                    <TableRow key={idx} sx={{ '&:hover': { background: 'rgba(255,255,255,0.02)' } }}>
                                        ...
                                        <TableCell sx={{ opacity: 0.5 }}>{new Date(entry.createdAt).toLocaleDateString()}</TableCell>
                                        <TableCell sx={{ fontWeight: 700, opacity: 0.8 }}>{(entry.planId || 'MONTHLY').toUpperCase()}</TableCell>
                                        <TableCell align="right" sx={{ fontWeight: 900 }}>₹{entry.price}</TableCell>
                                        <TableCell align="center"><Chip label="PAID" size="small" sx={{ fontWeight: 800, bgcolor: 'rgba(74,222,128,0.1)', color: '#4ade80' }} /></TableCell>
                                    </TableRow>
                                )) : (
                                    <TableRow><TableCell colSpan={5} sx={{ textAlign: 'center', opacity: 0.5, py: 5 }}>No previous settlements discovered.</TableCell></TableRow>
                                )}
                            </TableBody>
                        </Table>
                    </TableContainer>
                    ...
                    <Grid container spacing={4}>
                         <Grid item xs={12} md={6}>
                             <Box sx={{ p: 4, borderRadius: '24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                                 <Typography variant="h6" fontWeight={800} gutterBottom>Sovereign Payment Gateway Link</Typography>
                                 <Typography variant="body2" sx={{ opacity: 0.5, mb: 3 }}>The active link triggered by "Proceed to Pay". Updates are reflected across all B2B enterprise hubs immediately.</Typography>
                                 <TextField 
                                    fullWidth 
                                    value={gatewayLink}
                                    onChange={(e) => setGatewayLink(e.target.value)}
                                    placeholder="https://rzp.io/l/..."
                                    sx={{ mb: 3, '& .MuiOutlinedInput-root': { borderRadius: '12px', background: 'rgba(255,255,255,0.03)' } }}
                                    InputProps={{ startAdornment: (<InputAdornment position="start"><LinkIcon color="primary" /></InputAdornment>) }}
                                 />
                                 <Button variant="contained" onClick={handleUpdateGateway} sx={{ borderRadius: '12px', fontWeight: 900, px: 4 }}>SAVE GLOBAL CONFIGURATION</Button>
                             </Box>
                         </Grid>
                    </Grid>
                </Paper>
