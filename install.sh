...
# 6. Inject DB_PASSWORD into GeoSurePath config
echo "🔑 Injecting database password into GeoSurePath config..."
...
# 4b. Force GeoSurePath registration setting in database
echo "🔓 Enabling GeoSurePath user registration in database..."
# Wait for GeoSurePath to initialize schema (up to 30s)
for i in {1..15}; do
    if docker exec geosurepath_db psql -U ${DB_USER:-geosurepath} -d ${DB_NAME:-geosurepath} -c "SELECT 1 FROM tc_users LIMIT 1;" >/dev/null 2>&1; then
        echo "✅ GeoSurePath schema ready."
        break
    fi
    echo "⏳ Waiting for GeoSurePath schema... ($i/15)"
    sleep 2
done
...
# GeoSurePath 6.x hash for 'admin'
echo "👤 Checking GeoSurePath administrator..."
